...
#define DEBOUNCE_TIME 2 // 去抖动时间，单位：毫秒
#define LONG_PRESS_TIME 20 // 长按时间，单位：毫秒
#define DOUBLE_CLICK_TIME 10 // 双击间隔时间，单位：毫秒
// 定义按键引脚
...
typedef enum {
    BUTTON_IDLE,           // 空闲状态
    BUTTON_DEBOUNCE,       // 去抖动状态
    BUTTON_PRESSED,        // 按下状态
    BUTTON_RELEASED,       // 释放状态
    BUTTON_LONG_PRESSED,   // 长按状态
    BUTTON_DOUBLE_CLICKED  // 双击状态
} ButtonState;
...
volatile uint32_t button5PressTime = 0;
volatile uint32_t button6PressTime = 0;

volatile uint32_t button1ReleaseTime = 0;
volatile uint32_t button2ReleaseTime = 0;
volatile uint32_t button3ReleaseTime = 0;
volatile uint32_t button4ReleaseTime = 0;
volatile uint32_t button5ReleaseTime = 0;
volatile uint32_t button6ReleaseTime = 0;

volatile uint32_t button1LastClickTime = 0;
volatile uint32_t button2LastClickTime = 0;
volatile uint32_t button3LastClickTime = 0;
volatile uint32_t button4LastClickTime = 0;
volatile uint32_t button5LastClickTime = 0;
volatile uint32_t button6LastClickTime = 0;
...
void EXTI0_IRQHandler(void)
{
    if (EXTI_GetITStatus(EXTI_Line0) != RESET)
    {
        ...
        button1State = BUTTON_PRESSED;
				button1PressTime = SysTick->VAL; // 记录按下时间
...
void EXTI1_IRQHandler(void)
{
     if (EXTI_GetITStatus(EXTI_Line1) != RESET) {
        ...
        button2PressTime = SysTick->VAL; // 记录按下时间
    ...
    if (EXTI_GetITStatus(EXTI_Line2) != RESET)
        {
					button3State = BUTTON_PRESSED;
					 button3PressTime = SysTick->VAL; // 记录按下时间
          printf("REC button pressed\r\n");// 调试输出
					LCD_Init();
				disp_All();
...
void EXTI3_IRQHandler(void)
{
    ...
    if (EXTI_GetITStatus(EXTI_Line3) != RESET)
    {
        ...
        button4State = BUTTON_PRESSED;
			 button4PressTime = SysTick->VAL; // 记录按下时间
...
void EXTI4_IRQHandler(void)
{
    ...
    if (EXTI_GetITStatus(EXTI_Line4) != RESET)
    {
        button5State = BUTTON_PRESSED;
				 button5PressTime = SysTick->VAL; // 记录按下时间
...
void EXTI9_5_IRQHandler(void)
{
    ...
    if (EXTI_GetITStatus(EXTI_Line5) != RESET)
    {
        button6State = BUTTON_PRESSED;
			 button6PressTime = SysTick->VAL; // 记录按下时间
				printf("P=0 button pressed\r\n"); // 调试输出
			Buzzer_Off();
...
{
//	static uint32_t lastTick = 0; // 上一次去抖时间
    uint32_t currentTick = SysTick->VAL; // 当前时间
    ...
    if (button1State == BUTTON_PRESSED)
    {
        ...
        if (GPIO_ReadInputDataBit(BUTTON_GPIO_PORT, BUTTON1_PIN) == Bit_RESET)
        {
            ...
            else
            {
							 ...
							 // 处理按键5长按逻辑：开/关机
        Toggle_Power();
        printf("Toggle_Power run\r\n");
        button1State = BUTTON_RELEASED;
//                // 处理按键1短按逻辑：开关背光
//                //Toggle_Backlight();
//								printf("Toggle_Backlight run\r\n");
//                button1State = BUTTON_RELEASED;
            }
        }
    }
    ...
    if (button2State == BUTTON_PRESSED) {
        ...
        if (GPIO_ReadInputDataBit(BUTTON_GPIO_PORT, BUTTON2_PIN) == RESET) {
            if (currentTick - button2PressTime >= LONG_PRESS_TIME) {							
                button2State = BUTTON_LONG_PRESSED;//长按标志位
            } else if (button2PressTime - button2LastClickTime <= DOUBLE_CLICK_TIME) {
                button2State = BUTTON_DOUBLE_CLICKED;//双击标志位
                button2LastClickTime = 0;
            } else {
                button2State = BUTTON_RELEASED;
            }
        } else {
            button2State = BUTTON_IDLE;
        }
    }
    if (button2State == BUTTON_RELEASED) {
        if (button2PressTime - button2LastClickTime <= DOUBLE_CLICK_TIME) {
            button2State = BUTTON_DOUBLE_CLICKED;
        } else {
            button2State = BUTTON_IDLE;
            button2LastClickTime = button2PressTime;
        }
    }
    ...
    if (button2State == BUTTON_DOUBLE_CLICKED) {
        // 处理按键2双击逻辑
        printf("Button 2 Double Clicked\n");
        button2State = BUTTON_IDLE;
    }
