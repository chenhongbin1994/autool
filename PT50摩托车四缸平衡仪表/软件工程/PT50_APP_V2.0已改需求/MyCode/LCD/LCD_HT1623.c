 ...
 * @LastEditTime: 2024-08-19 19:43:08
...
//通信方式都是串口通信，这里是模拟SPI的通讯方式写入数据，HT1623指令配置根据自己需求添加修改*/
...
Discription:先写该SEG脚上前4位后SEG+1写4各位
Parameter: SEG 要写入的地址
		   dat 要写入的数据
************************************************/
...
void W1623_RPMbyte(uint8_t SEG, uint8_t dat)//转速数码段4,5管
{
	uint8_t i, addr;
	addr = SEG ;
	for (i = 0; i < 2; i++)
...
void W1623_LowerbyteMyriabit(uint8_t SEG, uint8_t dat)//控制2个SEG写低四位，控制一号缸(万位）数码段
{
	uint8_t i, addr;
	addr = SEG ;
	for (i = 0; i < 2; i++)
...
unsigned char HT1623_ReadData_Lcd(void)
{
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;//是否选浮空
    GPIO_Init(LCD_B_Port, &GPIO_InitStructure);
    // 配置数据引脚为输入模式
    ...
    GPIO_InitStructure.GPIO_Pin = LCD_B_Data_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//少了这行会默认模拟输入,可能会导致驱动能力不足，引脚电平无法达到预期的高/低电平。
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(LCD_B_Port, &GPIO_InitStructure);
 ...
 * @param mask 要修改的值
 * @param opr   操作类型（1 表示或操作，0 表示与操作）
 */
void UpdateReadModifyWriteDisplay(uint8_t address, uint8_t mask, uint8_t opr) {
    ...
    if (opr)
        ...
        ram_temp ^= mask;// 设置特定位,等同ram_temp &=  ~mask;
#endif
#if 1
    if (opr)
        ram_temp |=  mask;// 设置特定位
        ...
        ram_temp &=  ~mask;// 设置特定位
#endif 
    W1623_bothSeg_byte(address, ram_temp); // 将修改后的值写回
}
...
{	
	...
	GPIO_SetBits(LCD_B_Port, LCD_B_LED_PIN);UpdateReadModifyWriteDisplay(15,Screen_Data_LED,1); // 开背光以及图标
	if(current_settings.buzzer_enabled == 1)
		UpdateReadModifyWriteDisplay(15,Screen_Data_BUZZER,1);//点亮蜂鸣器图标
}
