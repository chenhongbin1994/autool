...
typedef enum {
    SETTING_UNIT,
    SETTING_SENSOR_COUNT,     //传感器数量
    SETTING_STROKE2T4T_MODE,       // 2冲程或4冲程模式设置项
...
typedef struct {
    ...
    int sensor_count;     // 传感器数量（0: 2T, 1: 4T）每次下载新机，都是从0开始，可以试试解决方案int sensor_count = 3;
    int stroke2T4T_mode;      // 0表示2冲程，1表示4冲程
