...
void SwitcUint(float* inputPressure)//单位换算
{
    ...
    switch(current_settings.current_unit)
    {
        case 0: // Pa
            ...
            for (int i = 0; i < 4; i++) {
                convertedPressure[i] = inputPressure[i] * 100; 
                printf("Pressure %d: AD_Value= %d, 压力= %.2f HPa\r\n", i, ADC_Value[i], convertedPressure[i]);
                //printf("Pressure %d: 压力= %.2f Pa\r\n", i,convertedPressure[i]);               
            }
            decimal_points = 0; // 不需要小数点
            break;
        ...
        case 6: // inH2O    2小数点   这个单位立项报告没要求
            UpdateReadModifyWriteDisplay(23, Switching_Unit_Data_inH2O, 1);
            for (int i = 0; i < 4; i++) {
                convertedPressure[i] = inputPressure[i] * 40.1463;//0.401463
                printf("Pressure %d: AD_Value= %d, 压力= %.2f HPa\r\n", i, ADC_Value[i], convertedPressure[i]);
...
void SettingsUint(float display_value)
{
    ...
    switch(current_settings.current_unit)
    {
        ...
        case 6: // inH2O    1小数点   这个单位立项报告没要求
            UpdateReadModifyWriteDisplay(23, Switching_Unit_Data_inH2O, 1);           
            temp_value = display_value * 40.1463;
            //temp_value = temp_value*10;            
            decimal_points = 2; 
            break;
