...
// 修改的USB数据接收函数
extern  u16 applenth;
u8 firmware_received = 0;       // 标记是否收到完整固件
u32 firmware_total_size = 33812; // 固件总大小（需根据实际情况设置）
u32 firmware_received_size = 0; // 已接收固件大小
u8 firmware_buffer[40000];      // 固件接收缓冲区
void reset_firmware_update_state(void)
{
    firmware_received = 0;
...
void USB_To_USART_Send_Data(uint8_t* data_buffer, uint8_t Nb_bytes)
{
    if (firmware_received) {
        return; // 已接收到完整固件，忽略后续数据
    }
    
    // 检查接收缓冲区是否会溢出
    if ((firmware_received_size + Nb_bytes) > firmware_total_size) {
        // 错误处理：接收到的数据超出预期大小
        usb_printf("接收到的数据超出预期大小\r\n");
        firmware_received_size = 0;
        return;
    }

    // 将数据复制到接收缓冲区
    memcpy(&firmware_buffer[firmware_received_size], data_buffer, Nb_bytes);
    firmware_received_size += Nb_bytes;

    // 检查是否接收到完整的固件
    if (firmware_received_size >= firmware_total_size) {
        firmware_received = 1; // 设置接收完成标志

        // 开始进行固件更新
        iap_write_appbin(FLASH_APP1_ADDR, firmware_buffer, firmware_total_size);
        usb_printf("固件更新完成!\r\n");

        // 重置接收状态
        firmware_received_size = 0;
				//NVIC_SystemReset();	//更新完成关机，需要自己开机
    }
}
