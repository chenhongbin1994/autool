...
#define KEY_LONG_PRESS_TIME    200 // 5ms*200 = 1s
#define KEY_WAIT_DOUBLE_TIME   1 // 5ms*50 = 250ms，如果没开双击就填1，开双击填300或者500
...
KEY_Configure_TypeDef Key0Cfg = {0, KEY_Action_Press, KEY_Status_Idle, KEY_Event_Null, KEY_ReadPin0, 0};
...
{ 
	...
	GPIO_InitStructure.GPIO_Pin =KEY_PIN_1 | KEY_PIN_2 | KEY_PIN_3 | KEY_PIN_4 | KEY_PIN_5;
	...
	GPIO_Init(KEY_PORT, &GPIO_InitStructure);
		...
		GPIO_InitStructure.GPIO_Pin = KEY_PIN_0;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//少了这行可能会导致驱动能力不足，引脚电平无法达到预期的高/低电平。
	...
	else
	{
		...
		switch(KeyCfg->KEY_Status)
		{
			...
			case KEY_Status_ConfirmPressLong:
				...
				{		// 一直等待其放开
					KeyCfg->KEY_Status = KEY_Status_ConfirmPressLong;
					KeyCfg->KEY_Event = KEY_Event_Null; // 长按状态时不重复标记事件KEY_Event_LongPress|KEY_Event_Null
					//KeyCfg->KEY_Event = KEY_Event_LongPress; // 长按状态时重复标记事件
					KeyCfg->KEY_Count = 0;
				}
				else
				{
